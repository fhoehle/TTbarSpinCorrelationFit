...
public:
  ...
  SpinFitSyst();
  SpinFitSyst(string noCorr, string Corr);
  ~SpinFitSyst(){};
  void setup();
...
SpinFitSyst::SpinFitSyst()
{
  //fileWithCorrelation = "test_MCatNLO.root";
  fileWithCorrelation = "ttbar_mcatnlo_SC_ref.root";
  filenoCorrelation = "ttbar_mcatnlo_SC_ref.root";
  
  fileWithCorrelation_gen = "ttbar_mcatnlo_SC_ref.root";
  fileNoCorrelation_gen = "ttbar_mcatnlo_SC_ref.root";
  
  
  /*
  fileWithCorrelation = "TT_powheg_SC.root";
  filenoCorrelation = "TT_powheg_NoSC.root";
  
  fileWithCorrelation_gen = "TT_powheg_SC.root";
  fileNoCorrelation_gen = "TT_powheg_NoSC.root";
  */
  
  /*
  fileWithCorrelation = "TT_mcatnlo_m170_NoSC.root";
  filenoCorrelation = "TT_mcatnlo_m170_SC.root";
  
  fileWithCorrelation_gen = "TT_mcatnlo_m170_NoSC.root";
  fileNoCorrelation_gen = "TT_mcatnlo_m170_SC.root";
   */
...
//   fileWithCorrelation = "templates/TTbarSpinCor1.root";
//   filenoCorrelation = "templates/TTbarNoSpinCor1.root";
//   fileWithCorrelation_gen = "templates/TTbarSpinCor2.root";
//   fileNoCorrelation_gen = "templates/TTbarNoSpinCor2.root";


  setup();
}

SpinFitSyst::SpinFitSyst(string noCorr, string Corr)
{
  fileData = "background_mcatnlo_NoSC.root";
  fileBackground = "background_mcatnlo_NoSC.root";
...
void SpinFitSyst::assembleDatasetsHisto()
{
...
void SpinFitSyst::assembleDatasets(map<string,RooDataHist*> mapToImport)
{
  if (data) delete data;
  if (debug) cout << "Combine "<< maxType<<" categories\n";
  data = new RooDataHist("data","combined data",*deltaPhi,Index(*sample),
...
void SpinFitSyst::printAllValues()
{
  char hname[150];
  for (int itype = 0;itype!=maxType;++itype) {
    if (useRatio) {
  ...
  << " , +"<<5*w->var("ratioCorr")->getErrorHi()<<" )\n";
    ...
    cout << "the spin corr is  " << (5*(w->var("ratioCorr")->getVal())-2)*0.33 << "  pm " << 5*0.33*w->var("ratioCorr")->getError()<< endl;
  //ratioCorr->Print();

}
void SpinFitSyst::fixBackground(int fix)
{
  char hname[50];
   switch ( fix )
   {
...
pair<double,double> SpinFitSyst::findMin()
{
...
void SpinFitSyst::printLL(bool toFile, char* name)
{
  c_min->cd();
...
void SpinFitSyst::printFit(bool withFit, bool toFile, char* name)
{
  char hname[50];
...
void SpinFitSyst::typeTag(char *name)
{
  char name2[200];
  for (int j =0; j<maxType;++j) {
...
void SpinFitSyst::calibration(int numberPE, float lowerLimit, float upperLimit,
	int numberBins)
{
  time_t start,end;
  time (&start);
...
void SpinFitSyst::pdfSyst(int numberPE, float ratioCorrGen)
{
  time_t start,end;
  time (&start);
...
void SpinFitSyst::do_toys(int numberPE, float ratioCorrGen, int pdfToUse)
{
  quietFit(true);
  ...
  toy_mean   = new TH1F("mean"  ,"Ratio",100, ratioCorrGen-0.3, ratioCorrGen+0.3);
...
void SpinFitSyst::nicePlot()
{
  ...
  sumData->GetXaxis()->SetTitle("#Delta #phi");
  ...
  leg->AddEntry(totalNoCorr, "Total, no correlation", "l");
  leg->AddEntry(totalSM, "Total, SM", "l");
...
int SpinFitSyst::generate_toy(float ratioCorrGen, int pdfToUse)
{
...
//       }
      ...
      float moyenne =0.;
      if (itype==0) moyenne = 1069.03;//1724.59;
      else if (itype==1) moyenne = 4080.18;//6382.7;
     else if (itype==2) moyenne =1370.3;// 2142.39;
...
void SpinFitSyst::generateComponent(RooDataHist *rooDataHist, char * hname, int nbr)
{
  if (nbr!=0) {
  //cout << hname<<" "<<nbr<<endl;
...
int main(int argc, const char* argv[])
{
  ...
  if (systematics) {
   if (argc<3) {
    ...
    j = new SpinFitSyst(argv[1],argv[2]);
  } else {
    j = new SpinFitSyst();
  }   
